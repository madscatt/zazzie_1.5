...
def calc_chisq(sas_folders, file_iq_interpolated, x2outfile, output_prefix=None,
                              show=False, sas_ext='iq'):

    #assert len(sas_folders) == 1, ("ERROR: mode for examining multiple "
    #                               "folders not currently tested")

    if not output_prefix:
        output_prefix = 'sas_x2'
    ...
    # Read reference i(q) values from interpolated i(q)'s file
    file = open(file_iq_interpolated,"r").readlines()
    x2out=open(x2outfile,'w')
    x2out.write('# file generated on \n# structure, X2 \n')
    iq_goal = []
    sig = []
    for line in file:
        iq = string.split(line)
        iq_goal.append(float(iq[1]))
        sig.append(float(iq[2]))

    cwd = os.getcwd()
    sas_search_path = os.path.join(cwd, sas_folders, '*.' + sas_ext)
    ...
    for name in file_list:
        sinf=open(name).readlines()
        sln = len(sinf)
        j = j + 1
        x2 = 0.0
        for q in xrange(sln):
            slin = string.split(sinf[q])
            iq = locale.atof(slin[1])
            diff = iq - iq_goal[q]
            diff2 = diff*diff/sig[q]
            x2 = x2 + diff2
        x2out.write('%d\t%f\n' % (j,x2))
