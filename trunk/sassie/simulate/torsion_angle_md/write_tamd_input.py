...
def get_rigid_regions(nregions,reslow,numcont,residues):
	...
	for i in xrange(nregions):
                print '!!!',i,nregions
		this_rigid_region = []
		### check and see if the last flexible region goes to the end of the protein
		if((i == nregions-1) and (flexible_regions[-1][-1] == residues[-1]) and (nregions > 1)):
			...
			#for residue in xrange(flexible_regions[i][-1]+1,flexible_regions[i+1][0]):
			#	this_rigid_region.append(residue)
...
def get_protein_cluster_strings(all_strings,segname,numranges,reslow,numcont,residue):
	...
	low_residue = residue[0] ; high_residue = residue[-1]

	residues = [i for i in range(low_residue,high_residue+1)]

###   assumes residue numbering increases 

	flexible_regions,rigid_regions = get_rigid_regions(numranges,reslow,numcont,residues)
	...
	#	print 'flexible regions : ',flexible_regions[i][0],':',flexible_regions[i][-1]
	...
	for i in xrange(len(rigid_regions)):
		...
		if((i == 0) and (rigid_regions[0][0]==residues[0])):
			...
			X = rigid_regions[i][0] ; Y = rigid_regions[i][-1]
			st = '\tcluster select segid '+segname+' .and. resid '+str(X)+':'+str(Y)+' .or. -\n'
			st = st + '\t\t(segid '+segname+' .and. resid '+str(Y+1)+' .and. ( type N .or. type HN ) ) end\n'
		#	print st
			...
			X = rigid_regions[i][0] ; Y = rigid_regions[i][-1]
			st = '\tcluster select segid '+segname+' .and. resid '+str(X)+':'+str(Y)+' .or. -\n'
			st = st + '\t\t(segid '+segname+' .and. resid '+str(X-1)+' .and. ( type C .or. type O ) ) end\n'
		#	print st
			cluster_string.append(st)
			all_strings.append(st)
		else:
			...
			X = rigid_regions[i][0] ; Y = rigid_regions[i][-1]
			st = '\tcluster select segid '+segname+' .and. resid '+str(X)+':'+str(Y)+' .or. -\n'
			st = st + '\t\t(segid '+segname+' .and. resid '+str(X-1)+' .and. ( type C .or. type O ) ) .or. -\n'
			st = st + '\t\t(segid '+segname+' .and. resid '+str(Y+1)+' .and. ( type N .or. type HN ) ) end\n'
		#	print st
			cluster_string.append(st)
			all_strings.append(st)
...
def get_cluster_strings_dna(cluster_file_name,flexible_regions,rigid_regions,resnum,resname,segres):
    ...
    # assuming dna pdb file is properly  renamed to match the residue name as in top/par files
    ...
    for i in xrange(len(rigid_regions)):
        if rigid_regions[i]:
            X = rigid_regions[i][0] ; Y = rigid_regions[i][-1]
            # if the rigid region is across different segment, cluster as two rigid bodies.
            for this_segres in segres:
                if ((X<this_segres) and (Y>=this_segres)):
                    st = '\tcluster select resid '+str(X)+':'+str(this_segres-1)+' end\n'
                    st = st+'\tcluster select resid '+str(this_segres)+':'+str(Y)+' end\n'
                else:
                    st = '\tcluster select resid '+str(X)+':'+str(Y)+' end'
    ...
    for i in xrange(len(flexible_regions)):
        if flexible_regions[i]:
            for flexible_resnum in flexible_regions[i]:
                ...
                st = "\t!"+str(flexible_resname)+","+str(flexible_resnum)+":\n"
                st = st + "\t!sugar\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type C1' .or. type H1' ) end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type C2' .or. type H2'' .or. type H2') end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type C3' .or. type H3' ) end\n"    
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type O3' .or. type H3T) end\n"         
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type C4' .or. type H4' ) end\n"                
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type C5' .or. type H5' .or. type H5'' ) end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. (type O5' .or. type H5T) end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and.  type O4' end\n"
                st = st + "\t!phosphate group\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. type P end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. type O1P end\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. type O2P end\n"
                st = st + "\t!base\n"
                st = st + "\t cluster select resid "+str(flexible_resnum)+" .and. -\n"  
                print "!!!!!test:",flexible_resname,"\n"
                if (flexible_resname=="ADE"):
                    st = st + "\t\t(type N1  .or. type C2 .or. type H2  .or. -\n"
                    ...
                    st = st + "\t\t(type H1 .or. type N1  .or. type C2  .or. -\n"
...
def get_rna_cluster_strings(all_strings,segname,numranges,reslow,numcont,residue,resnames):
	...
	for i in xrange(len(rigid_regions)):
		if rigid_regions[i]:
			X = rigid_regions[i][0] ; Y = rigid_regions[i][-1]
			st = '\tcluster select segid '+segname+' .and. resid '+str(X)+':'+str(Y)+' end'
			cluster_string.append(st)
	cluster_string.append("\n! flexible regions\n")
	for i in xrange(len(flexible_regions)):
		if flexible_regions[i]:
			for flexible_resnum in flexible_regions[i]:
				...
				st = "\t!"+str(flexible_resname)+","+str(flexible_resnum)+":\n"
				st = st + "\t!sugar\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C1' .or. type H1' ) end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C2' .or. type H2'') end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type O2' .or. type H2') end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C3' .or. type H3' ) end\n"    
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type O3' .or. type H3T) end\n"         
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C4' .or. type H4' ) end\n"                
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C5' .or. type H5' .or. type H5'' ) end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type O5' .or. type H5T) end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and.  type O4' end\n"
				st = st + "\t!phosphate group\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. type P end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. type O1P end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. type O2P end\n"
				st = st + "\t!base\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. -\n"                
				if (flexible_resname=="ADE"):
					st = st + "\t\t(type N1  .or. type C2 .or. type H2  .or. -\n"
					st = st + "\t\t type N3  .or. type C4 .or. type C5  .or. -\n"
					st = st + "\t\t type C6  .or. type N6 .or. type H61 .or. -\n"
	...
	for i in xrange(len(flexible_regions)):
		if flexible_regions[i]:
			for flexible_resnum in flexible_regions[i]:
				...
				st = st + "\t!sugar\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C1' .or. type H1' ) end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C2' .or. type H2'' .or. type H2') end\n"
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C3' .or. type H3' ) end\n"    
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type O3' .or. type H3T) end\n"         
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C4' .or. type H4' ) end\n"                
				st = st + "\t cluster select segid "+segname+" .and. resid "+str(flexible_resnum)+" .and. (type C5' .or. type H5' .or. type H5'' ) end\n"
...
def add_dna_patches(segname_molecule,input_strings):
	...
	for i in xrange(len(resnames)):
		...
		if(resnames[i] == "CYT" or resnames[i] == "THY"):
			input_strings.append("patch deo1 "+this_segname+" "+str(this_resid))
		elif(resnames[i] == "ADE" or resnames[i] == "GUA"):
			input_strings.append("patch deo2 "+this_segname+" "+str(this_resid))
...
def get_dna_rna_setup_strings(all_strings,segname,flex_variables,residue):
	...
	flexible_regions,rigid_regions = get_rigid_regions(numranges,reslow,numcont,residue)
        noest = []
        st = []
        noest='set dist = 1.52\n'
        noest=noest+'NOE\n'
        for i in xrange(len(flexible_regions)):
                if flexible_regions[i]:
                        for flexible_resnum in flexible_regions[i]: 
                                ...
                                st=st+'\t.and. resid '+str(flexible_resnum)+' .and. type C3\' end -\n'
                                st=st+'\tselect segid '+segname+' -\n'
                                st=st+'\t.and. resid '+str(flexible_resnum)+' .and. type C2\' end\n\n'
                                all_strings.append(st)
                                noest=noest+'\tassign select segid '+segname+' -\n'
                                noest=noest+'\t.and. resid '+str(flexible_resnum)+' .and. type C3\' end -\n'
                                noest=noest+'\tselect segid '+segname+' -\n'
                                noest=noest+'\t.and. resid '+str(flexible_resnum)+' .and. type C2\' end -\n'
...
def get_all_cluster_strings(all_strings,segname_molecules,flexible_segment_variables):
	...
	for i in xrange(len(segname_molecules)):
		...
		if(this_moltype == 'protein' and this_segname in flexible_segment_variables):	
			...
			get_protein_cluster_strings(all_strings,this_segname,numranges,reslow,numcont,residue)
			...
			get_rna_cluster_strings(all_strings,this_segname,numranges,reslow,numcont,residue,resnames)
			...
			numcont = flex_variables[2]
			#all_strings.append('\tcluster select segid '+this_segname+' end\n')
			get_dna_cluster_strings(all_strings,this_segname,numranges,reslow,numcont,residue,resnames)
                else:
                        all_strings.append('\tcluster select segid '+this_segname+' end\n')
...
def write_tamd_input(m1,segname_molecules,input_file_name,number_of_steps,dcdfreq,temp_pdb_files,topology_file_name,parameter_file_name,output_dcd_file_name,temperature,rgforce,rgvalue,residue,flexible_segment_variables,pretamd_min_steps):
	...
	all_strings.append("open read card unit 1 name "+topology_file_name+"\n")

	all_strings.append("read rtf card unit 1\nclose unit 1\n")
	
	all_strings.append("open read card unit 2 name "+parameter_file_name+"\n")
	...
	for i in xrange(len(temp_pdb_files)):
		...
		this_moltype = segname_molecules[i].moltype()[0]
                residue = segname_molecules[i].resid()	
		all_strings.append("read sequence pdb name "+pdb_file_name+" unit "+str(base_unit)+"\n")
		...
		if(this_moltype == 'protein'):	
			all_strings.append("generate "+this_segname+" setup warn\n")
                        ...
                        if (this_segname in flexible_segment_variables):
                                dnarnaflag = 1
			all_strings.append("generate "+this_segname+" setup warn first 5TER last 3TER\n")
		...
		if(i==0):
			all_strings.append("read coor pdb name "+pdb_file_name+" sele segid "+this_segname+" END\n")
		else:
			all_strings.append("read coor pdb name "+pdb_file_name+" sele segid "+this_segname+" END APPEND\n")
		...
		if(this_moltype == 'dna' or this_moltype == 'rna' and this_segname in flexible_segment_variables):
                        get_dna_rna_setup_strings(all_strings,this_segname,flexible_segment_variables[this_segname],residue)
	...
	all_strings.append("coor copy comp\nic build comp\ncoor copy select .not hydrogen end")
	all_strings.append("hbuild atom cdie eps 80 cutnb 10.0 ctofnb 7.5 ctonnb 6.5 shift vshift bygr\n")
	all_strings.append("faster on\n")
	all_strings.append("update atom rdie eps 4 cutnb 20 ctofnb 18 ctonnb 16 shift vshift bygr\n")    
	...
	all_strings.append("energy\n")
        ...
        all_strings.append("\tGEO sphere rcm force 5.0 droff 0.0 xref @xcm yref @ycm zref @zcm -\nsele all end\n")
        all_strings.append("END\n")
        if auto_directed_rg:
	...
	all_strings.append("end\n")
        ...
        all_strings.append("mini sd   nstep "+pretamd_min_steps+" nprint 10 step 0.01\n")
        all_strings.append("mini abnr nstep "+pretamd_min_steps+" nprint 10 step 0.01\n")
	
	all_strings.append("tamd\n")
	all_strings.append("\tbomlev -1")     

	get_all_cluster_strings(all_strings,segname_molecules,flexible_segment_variables)
	...
	all_strings.append("\topen write unit 131 file name "+output_dcd_file_name+"\n")
...
#	close unit 2
	...
	all_strings.append("\t\tnstep "+str(number_of_steps)+" timestep 0.002 qref 20 tref "+str(temperature)+" first "+str(temperature)+ " -")
	#all_strings.append("\t\tnsavc 500 nprint 10000 iprfrq 100000 nsavv 0 isvfrq 2000 -")
	all_strings.append("\t\tnsavc "+str(dcdfreq)+" nprint 10000 iprfrq 100000 nsavv 0 isvfrq 2000 -")
