...
class Setup():
	...
	def chooser(self,coor,m1,pairdat,dtheta,numranges,reslow,numcont,dihedral_parameters,beta,residue_rotation_indices,residue_rotation_mask,nonbondflag,first_last_resid,molecule_type,seed_object):
		...
		# pick a random angle for the list of dihedral angles for this molecule type
		...
		if(seed_object != -1):
			...
			if(molecule_type == 'rna'):
				# chose an angle type that does not include the delta dihedral angle
				while(search):
		...
		# set specific dihedral parameters in this range (idum) for this residue (ndum)
		if(molecule_type == 'protein'):
			resphi=dihedral_parameters[0] ; respsi=dihedral_parameters[1]
        		parm=[]
			...
			resalpha=dihedral_parameters[0] ; resbeta=dihedral_parameters[1]
			resgamma = dihedral_parameters[2] ; resdelta=dihedral_parameters[3] 
			resepsilon=dihedral_parameters[4] ; reseta=dihedral_parameters[5] 
        		parm=[]
        		parm=[resalpha[idum][1][ndum],resbeta[idum][1][ndum],resgamma[idum][1][ndum],resdelta[idum][1][ndum],resepsilon[idum][1][ndum],reseta[idum][1][ndum]]
			
			rna_dihedrals = ['alpha','beta','gamma','delta','epsilon','eta']
			an = rna_dihedrals[this_dihedral]

		vdi,vdf,trial_theta=self.find_angle(coor,an,indices,this_dihedral,q0,this_dtheta,beta,parm,this_mask,nonbondflag,first_last_resid,molecule_type,seed_object)
	...
	def find_angle(self,coor,an,indices,this_dihedral,q0,this_dtheta,beta,parm,this_mask,nonbondflag,first_last_resid,molecule_type,seed_object):
       ...
       # 	angle_value=sascalc.Measure.calculate_dihedral(coor,this_sub_mask)
		angle_value=dihedral_rotate.measure(coor,indices,an,this_mask,q0,first_last_resid,molecule_type)
		search=1
		while(search==1):
			if(seed_object != -1):
				ran_num = seed_object.rand()
			...
			search,vdi,vdf=dihedral_energy.calc(this_dihedral,angle_value,theta,parm,beta,nonbondflag,seed_object)
