...
def nmer_overlap_check(m1,path,pdbfile,cutoff,basis,keyword_basis):
	...
	for i in range(npairs):
		if not keyword_basis:
			a = str(basisall[i][0])
			b = str(basisall[i][1])
			mol1_basis = 'name[i] == "'+a+'" and segname[i] == "'+pair[i][0]+'"'
			mol2_basis = 'name[i] == "'+b+'" and segname[i] == "'+pair[i][1]+'"'
		else:
		### TODO --> all not attempted yet
			...
			#	basis_filter = 'not name[i] == None'
				...
				mol1_basis = '(not name[i][0] == "H") and segname[i] == "'+pair[i][0]+'"'
				mol2_basis = '(not name[i][0] == "H") and segname[i] == "'+pair[i][1]+'"'
			else:
