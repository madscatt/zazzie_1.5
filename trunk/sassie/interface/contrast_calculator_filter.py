...
#           DNA can be classified as 'rna' here and get through because only THY is classified as 'dna' in SasMol
            ...
            fastaseq = contrast_calculator.FASTA_sequence(pdbfile)
            if not fastaseq:
                error.append('The FASTA file "'+pdbfile+'" appears to be empty.')
                return error

            if allmoltype[i] == 'dna':
                fastadict = contrast_calculator.dna_sl()
            elif allmoltype[i] == 'rna':
                fastadict = contrast_calculator.rna_sl()
            elif allmoltype[i] == 'protein':
                fastadict = contrast_calculator.protein_sl()
            else:
                error.append('The molecule type for input file '+str(i+1)+' must be DNA, RNA or protein.')
                return error


#           invalid letter codes that aren't protein, rna or dna are discarded in contrast_calculator.FASTA_sequence above
