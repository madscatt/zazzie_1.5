...
#    0.000000        0.019000        0.000824
    ...
    if number_spec_files==0:
        error.append("there are no scattering files found for the selected sas-type: '"+name_sastype+"' in folder: '"+saspath+"'")
        return error
    # check q values
    Q_spec = []
    ...
    if len(spec_files)>1:
        for f in spec_files[1:]:
            if os.path.getsize(f) != file_size:
                ...
                for line in this_spec_files:
                    lin = string.split(line)
                    q_in_spec_file.append(lin[0])
    
                if len(q_in_spec_file_0) != len(q_in_spec_file): 
                    error.append('number of q-values of "'+f+'" does not match "'+os.path.basename(spec_files[0])+'".\nPlease verify the files in the SAS path are consistent!')
                    return error
                elif q_in_spec_file_0 != q_in_spec_file: 
                    error.append('q-values of "'+f+'" do not match "'+os.path.basename(spec_files[0])+'".\nPlease verify the files in the SAS path are consistent!')
                    return error
