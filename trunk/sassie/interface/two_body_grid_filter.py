...
def check_input_values(variables):
	...
	if(accpos != 0 and accpos != 1):
		...
		return error
        	...
        	error.append( 'three float values are required for initial position (x,y,z) : ')
		return error
	elif(len(trans)!=3):
        	error.append( 'three int values are required for number of x,y,z moves : ')
		return error
	elif(trans[0] < 1 or trans[1] < 1 or trans[2] < 1):
        	error.append( 'you must specifiy at least ONE translational "move" for each axis : ')
		return error
	elif(len(dtrans)!=3):
        	error.append( 'three float values are required for dx,dy,dz step sizes : ')
		return error
	elif(len(theta)!=3):
        	error.append( 'three int values are required for theta angular moves : ')
		return error
	elif(theta[0] < 1 or theta[1] < 1 or theta[2] < 1):
        	error.append( 'you must specifiy at least ONE angular "move" for each axis : ')
		return error
	elif(len(dtheta)!=3):
        	error.append( 'three float values are required for dtheta (x,y,z) step sizes : ')
		return error
	elif(basis!='CA'):
		...
		return error
	elif(len(numcont1) != nexsegments1):
		...
		return error
	elif(len(numcont2) != nexsegments2):
