...
def check_psegvariables_exist(psegvariables,glbls):
	...
	if psegvariables not in glbls:
		error.append('Please enter the dihedral variables by clicking on the button "click to enter dihedral variables"!')
	return error
...
def check_segvariables(pdbfile,psegvariables,flpsegname,seglow,seghigh):
	...
	for i in range(len(allsith)):
		...
		for i in range(len(linith)):
			...
			try:
				...
				error.append('The angle value "'+tith+'" should be a float type!')
				return error
			if(fith>180.0 or fith<=0.0):
				error.append('The angle value "'+tith+'" should be in the range of (0.0,180.0]!')
				return error
			'''
			if(fith>180.0):
				fith=180.0
	...
	for i in range(len(allsnumranges)):
		try:
			...
			error.append('The number of ranges "'+allsnumranges[i]+'" for flexible segment number '+str(i)+' in the flexible segment input fields should be an integer type!')
			return error
		if nr<1:
			error.append('The number of ranges "'+allsnumranges[i]+'" for flexible segment number '+str(i)+' in the flexible segment input fields should be equal/greater than 1!')
			return error
	...
	for i in range(len(allsrlow)):
		...
		for k in range(len(linrlow)):
			try:
				...
				error.append('The low resid "'+allsrlow[i]+'" for flexible segment number '+str(i)+' in the flexible segment input fields should be an integer array!')
				return error
			rlow.append(trlow)
		for k in range(len(linrnum)):
			try:
				...
				error.append('The number of continuous residues "'+allsrnum[i]+'" for flexible segment number '+str(i)+' in the flexible segment input fields should be an integer array!')
				return error
			rnum.append(trnum)
		arlow.append(rlow)
		arnum.append(rnum)
	...
	for i in range(len(psegvariables)):
		...
		if seg_moltype.strip() not in moltype:
			error.append('The molecule type "'+seg_moltype+'" provided for flexible segment number '+str(i)+' in the flexible segment input fields does not match the pdb file!')
			return error
		...
		if(len(dtheta) != numranges):
        		error.append( 'the number of dtheta values does not match the number of ranges, dtheta = '+str(dtheta)+' numranges = '+str(numranges))
			return error
		elif(len(reslow) != numranges):
        		error.append( 'the number of low residue values does not match the number of ranges, lowres = '+str(reslow)+' numranges = '+str(numranges))
			return error
		elif(len(numcont) != numranges):
        		error.append( 'the number of contiguous residues does not match the number of ranges, lowres = '+str(numcont)+' numranges = '+str(numranges))
			return error
		...
		for j in xrange(numranges):
			if (reslow[j] not in resid):
				error.append('Input pdb file, '+str(pdbfile)+' does not have low residue amino acid, "'+str(reslow[j])+'" for segment number '+str(j)+', range = '+str(resid[0])+' : '+str(resid[-1]))
				return error
			elif (reslow[j]+numcont[j] not in resid):
				error.append('Input pdb file, '+str(pdbfile)+' does not have low+continuous residue amino acid, "'+str(reslow[j]+numcont[j])+'" for segment number '+str(j)+', range = '+str(resid[0])+' : '+str(resid[-1]))
				return error
			elif (numcont[j]<=0):
				error.append('The number of continuous residues "'+str(numcont[j])+'" should be greater than 0!')
				return error	
		...
		for j in xrange(numranges-1):
			if(reslow[j]< 2):
				error.append('low residue can not include the n-terminus, reslow = '+str(reslow))
				return error
			elif(reslow[j]>reslow[j+1]):
				error.append('low residue values must increase from low to high, reslow = '+str(reslow[j]))
				return error	
			elif(reslow[j]+numcont[j] > reslow[j+1]):
				error.append('low residue values plus number contiguous overlap, reslow = '+str(reslow[j])+' numcont = '+str(numcont[j]))
				return error	
			elif(reslow[j]+numcont[j] > number_aa-1):
				error.append('your low residue plus number contiguous exceed the number of amino acids ('+str(number_aa)+'), reslow = '+str(reslow[j])+' numcont = '+str(numcont[j]))
				return error	
		if(reslow[-1]+numcont[-1] > number_aa-1):
			error.append('your low residue plus number contiguous exceed the number of amino acids ('+str(number_aa)+'), reslow = '+str(reslow[-1])+' numcont = '+str(numcont[-1]))
			return error

	return error


def check_complex(variables,psegvariables,eflag,monflag,**kwargs):
    ...
    if(trials < 1):
        ...
        error.append( 'goback = '+str(trials)+'?')
        return error
    elif(temp < 0):
    ...
    #	return error
        ...
        error.append( 'zflag == 0 for "no" and 1 for "yes", zflag = '+str(zflag))
        return error
   # elif(zflag == 1):
   #     if zcutoff < 1.0:
   ...
   #         return error
        ...
        error.append( 'cflag == 0 for "no" and 1 for "yes", cflag = '+str(cflag))
        return error
    elif(cflag == 1):
        ...
        error.append('the number of segment '+str(nsegments)+' should be equal/greater than 1!')
        return error
    elif(npsegments<1):
        error.append('the number of flexible segment '+str(npsegments)+' should be equal/greater than 1!')
        return error
    elif (npsegments>nsegments):
        error.append('the number of flexible segment '+str(npsegments)+' should be equal/less than the number of total segments '+str(nsegments)+'!')
        return error
    ...
    if(segbasis.lower() != 'all' and segbasis.lower() != 'heavy' and segbasis.lower() != 'backbone'):
        if(len(segbasis.split(',')) != nsegments):
            error.append( 'the number of segment basis does not match the number of segments: number of segbasis = '+str(len(segbasis.split(',')))+' number of segments = '+str(nsegments))
            error.append('segment overlap basis entries can be "heavy", "backbone", "all", or a comma delimited list of atom names ... one for each segment')
            return error
        elif(len(flpsegname.split(',')) != npsegments):
            error.append( 'the number of flexible segment names does not match the number of flexible segments: number of flexible segment names = '+str(len(flpsegname.split(',')))+' number of flexible segments = '+str(npsegments))
            return error
        elif(len(seglow) != npsegments):
            error.append( 'the number of alignment low residues does not match the number of  flexible segments: number of alignment low residues = '+str(len(seglow))+' number of  flexible segments = '+str(npsegments))
            return error
        elif(len(seghigh) != npsegments):
            error.append( 'the number of alignment high residues does not match the number of  flexible segments: number of alignment high residues = '+str(len(seghigh))+' number of  flexible segments = '+str(npsegments))
            return error
...
###
        ...
        if(segbasis.lower() == 'backbone' or segbasis.lower() == 'heavy'):
            ...
            if any(None in sub_list for sub_list in atom_vdw):
                ...
                for i in xrange(m1.natoms()):
                    if(atom_vdw[i][0] == None):
                        atom_list += ' '+name[i]	
                error.append("the following atoms do not have vdW parameters; use different overlap basis : "+atom_list)
                return error		
            else:
                check_atoms = False
                cutoff = 0.8
            ...
            for i in xrange(len(string_basis)):
                test_atoms.append(string_basis[i].strip())	### for now ... early testing only
                cutoff = 2.0
...
### OPEN NEED OVERLAP ATOM CHECK FOR KEYWORDS HEAVY, BACKBONE, ETC.
    ...
    #### TODO ... alter this to use the correct basis for each segment !!!
        ...
        if(check == 1):
            error.append("your original structure has overlapping atoms using your chosen basis")
            error.append("\nif you fail to get accepted structures then either choose another basis or energy minimize your structure")
            print 'error = ',error 
            error = []
...
#			return error
    ...
    for item in flpsegname.split(','):
        if item.strip() not in m1.segname():
            error.append('The flexible segment name "'+item+'" is not found in the pdb file!')
            return error
    ...
    if(numsegs!=nsegments):
        error.append('the number of segments entered does NOT match the number of segments in the pdb file')
        return error
...
#            return error
    ...
    lerror = check_segvariables(pdbfile,psegvariables,flpsegname,seglow,seghigh)
    if len(lerror):
