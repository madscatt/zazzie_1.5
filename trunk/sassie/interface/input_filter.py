...
def type_check_and_convert(svariables):
    ...
    for key in svariables:
        ...
        if (svariables[key][1] == 'string'):
            ...
            if type(svariables[key][0]) is bool:
                variables[key] = svariables[key]
            elif type(svariables[key][0]) is str:
                ...
                try:
                    if value in true_list:
                        ...
                        error.append(
                            key + ': could not convert boolean input type [1]')
                        return error, variables
                except:
                    error.append(key + ': could not convert boolean input type [2]')
                    return error, variables

            else:
                error.append(key + ': could not convert boolean input type [3]')
                error.append(' : ' + str(type(svariables[key])))
                error.append(' : ' + str(svariables[key]))
                error.append(' : ' + str(svariables[key][0]))
                error.append(' : ' + str(svariables[key][1]))
                return error, variables
