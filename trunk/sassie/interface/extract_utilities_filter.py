...
#                       print(os.path.join(root, name))
        ...
        print 'sas_input_paths in filter: ', sas_input_paths 
        ...
        for sas_input_path in sas_input_paths:
            ...
            if(not ev or not rv):
                error.append('permission error in input file path ' +
                             sas_input_path + '  [code = ' + str(ev) + str(rv) + str(wv) + ']')
                if(ev == False):
                    error.append('sas path "' + sas_input_path + '" does not exist')
                    return error
                elif(rv == False):
                    error.append(
                        'read permission not allowed for sas path "' + sas_input_path + '"')
                    return error
            if(sas_type == 0):
                suffix = ['*.iq', '*.log']
                ...
                suffix = ['*.int', '*.log']
                extra = ['*.sav', '*.flm', '*.alm', '*.ans']
            spec_files = glob.glob(os.path.join(sas_input_path, suffix[0]))
            log_files = glob.glob(os.path.join(sas_input_path, suffix[1]))
        # spec_files.sort()
        # log_files.sort()
            extra_files_list = []
            for ex in extra:
                extra_files_list.append(
                    glob.glob(os.path.join(sas_input_path, suffix[0])))
            ...
            if number_spec_files == 0:
                error.append("there are no scattering files found for the selected sas-type: '" +
                             name_sastype + "' in folder: '" + sas_input_path + "'")
                return error
