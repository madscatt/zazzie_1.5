		...
		if(len(lin)>0):
			if(lin[0][0] != "#"):
				...
				if(i == 0):
					...
					message = 'repeadted z-value in experimental data: '+str(this_value)+'\n'
...
def check_sld_mol(variables,**kwargs):
    ...
    if(runtype != 0 and runtype != 1):
        error.append('run type entered needs to be eith 0 or 1 : '+str(runtype))
        return error
    elif(xon != 0 and xon != 1):
        error.append('scattering type (xon) entered needs to be eith 0 or 1 : '+str(xon))
        return error
    elif(plotflag != 0 and plotflag != 1 and plotflag != 2):
        error.append('plofflag needs to be 1, 2, or 3 : '+str(plotflag))
        return error
